...
def find_primes(start : int, tot : int, inc: int):
    ...
    while ct < tot:
        ...
        if isPrime(x) == True:
            ...
            print(f"Prime7.py {x} - Is Prime! count = {ct}. Duration = {duration:.2f}, primes per second = {(ct/duration):.4f} ")
        x += inc
...
def persist_prime(p: int, conn: pyodbc.Connection):
    try:
        with conn.cursor() as cursor:
            cursor.execute("EXEC dbo.Prime_Insert_7 @PrimeNumber = ?", p)
            conn.commit()
    except Exception as e:
        print(f"Error inserting prime number {p}: {e}")

find_primes(17, 13000, 10)

print("prime7.py - completed succesfully")
time.sleep(3)
